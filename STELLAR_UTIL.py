...
class STELLAR_UTIL:
    ...
    def log_filters_get(self, tenant_id=None):
        ret = {}
        path = "/connect/api/v1/log_filters"
        if tenant_id:
            path += "?cust_id={}".format(tenant_id)
        self.l.info("Getting log filters")
        r = self._request_get(path=path)
        ret = r.get('data', [])
        return ret

    def log_filters_update(self, filter_name, content=None, tenant_id=None):
        ret = {}
        update_data = {}
        filter_id = ''
        log_filters = self.log_filters_get(tenant_id=tenant_id)
        for log_filter in log_filters:
            if log_filter.get('name', '') == filter_name:
                filter_id = log_filter.get('_id', '')
                break
        if filter_id:
            self.l.info("Found filter: [{}] [id: {}]".format(filter_name, filter_id))

            if tenant_id:
                update_data['cust_id'] = tenant_id
            ''' stubbed - needs work'''
        else:
            self.l.warn("Log filter name not found: [{}] - update not possible".format(filter_name))
        # path = "/connect/api/v1/log_filters/{}".format(filter_id)
        # self.l.info("Updating log filters")
        # r = self._request_get(path=path)
        # ret = r.get('data', [])
        return ret
...
class local_db():
    ...
    def __init__(self, dbname='stellar_sync.db', ticket_table_name='tickets', optional_db_dir=None):
